...
function CreateItemForm() {
	...
	const handleSubmit = async (event) => {
		...
		for (let i = 0; i < formObjKeys.length; i++) {
			if (
				(formObjKeys[i].includes('price') || formObjKeys[i].includes('quantity')) &&
		...
		if (formValues.sizes_available && formValues.sizes_available.length > 0) {
			formValuesCopy.sizes_available = JSON.stringify(formValues.sizes_available);
		} else {
				...
				<Grid>
								...
								>
									{sizes.map((size) => (
										<MenuItem key={size} value={size}>
											<Checkbox
												checked={
													formValues.sizes_available.indexOf(size) > -1
												}
											/>
					...
					<div className='item-form-buttons'>
						...
						<Link to='/admin' style={{ textDecoration: 'none' }}>
							<Button variant='outlined' color='secondary' id='form-cancel-button'>
								Cancel
							</Button>
						</Link>
					</div>
